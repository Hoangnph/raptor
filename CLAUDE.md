# RAPTOR — Behavioral Guidelines + Project Instructions

> **Core behavioral guidelines** (from andrej-karpathy-skills) bias toward **caution over speed**.
> For trivial tasks (typos, obvious one-liners), use judgment to skip verbose verification loops.
...
# RAPTOR Project — Autonomous Offensive/Defensive Research Framework
...
## Project-Specific Guidelines (feat/exploit-db-web-scanning-upgrade)

### TDD Mandate
- **RED → GREEN → REFACTOR** for every new function/module
...
- No merge if coverage < 80%
- No merge if documentation incomplete
- Maximum 1 open TD, payback within 1 week

### Architectural Constraints
- **No cross-package imports** — packages only import from `core/`
- **Standalone executability** — each package runs independently
- **CLI interface** — argparse for every module
- **SARIF compatibility** — reuse `core.sarif.parser` for Nuclei output
- **Match existing style** — type hints, docstrings, logging patterns
...
- [ ] Only changed necessary lines (surgical changes)
